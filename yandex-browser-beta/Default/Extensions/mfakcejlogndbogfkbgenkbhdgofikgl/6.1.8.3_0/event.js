...
w.prototype.create = function() {
  var a = this, b = {type:"progress", title:"Lightshot", message:v.c("screenshot_plugin_uploading_window_capt"), iconUrl:"img/icon256.png", buttons:[{title:v.c("screenshot_plugin_cancel")}], isClickable:!0, progress:0};
  50 <= n().ea && (b.requireInteraction = !0);
...
function K(a, b) {
  ...
  this.C = "5CE3DF4D45AC";
  this.W = "http://upload.prntscr.com/upload/{time}/{hash}/";
}
s.extend(K, G);
m = K.prototype;
m.upload = function() {
  ...
  var b = Math.floor(Date.now() / 1E3), k = F("{key}*{time}".replace("{key}", this.C).replace("{time}", b)), b = this.W.replace("{time}", b).replace("{hash}", k);
...
var Q = function(a) {
  (function() {
    ...
    b.aa = "sha256-Ti7WNavwsty6w+oE0WzPWLshlTZNZbdhkPA9oPQyVcU=";
